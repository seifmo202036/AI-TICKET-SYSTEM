...
export async function claimTicket(ticketId: string, agentId: string) {
  ...
  try {
    ...
    const claimedTicket = await claimTicketRepo(ticketId, agentId, client);
...
export async function resolveTicket(ticketId: string, agentId: string) {
  const client = await pool.connect();
...
export async function closeTicket(ticketId: string, customerId: string) {
  const client = await pool.connect();
